...
library(vioplot)
library(limma)
expFile="TCGAexp.txt"       #输入文件
chemokinesFiles<-"chemokines.txt"
cliFile<-"TCGACliRiskscore.txt"
outpdf="vioplot-TCGA-chemokines.pdf"               #输出图片名称
...
write.table(chemoexp,file="chemoexp-TCGA.txt",sep="\t",quote=F,col.names=F)
chemoexp<-temp
chemoexp<-t(chemoexp)
data<-chemoexp
#读取msi或者risk信息
...
y=c(1:ncol(rt))
plot(x,y,
     xlim=c(0,pchnum*3),ylim=c(min(rt),max(rt)+0.02),
